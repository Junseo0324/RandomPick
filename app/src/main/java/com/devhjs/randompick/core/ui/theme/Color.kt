package com.devhjs.randompick.core.ui.theme

import androidx.compose.ui.graphics.Color

val SeedPrimary = Color(0xFF6366F1)
val SeedPrimaryLight = Color(0xFF818CF8)
val SeedPrimaryDark = Color(0xFF3730A3)

val SeedSecondary = Color(0xFFFBBF24)
val SeedSecondaryLight = Color(0xFFFCD34D)
val SeedSecondaryDark = Color(0xFFD97706)

val SeedError = Color(0xFFEF4444)
val SeedInfo = Color(0xFF3B82F6)

val Gray100 = Color(0xFFF3F4F6)
val Gray400 = Color(0xFF9CA3AF)
val Gray500 = Color(0xFF6B7280)
val Gray800 = Color(0xFF1F2937)

val LightBackgroundBase = Color(0xFFFAFAFA)
val DarkBackgroundBase = Color(0xFF121212)
val DarkSurfaceBase = Color(0xFF1E1E1E)

val DarkBackground = Color(0xFF09090B) // Zinc 950

val DarkSurface = Color(0xFF18181B) // Zinc 900
val DarkSurfaceVariant = Color(0xFF27272A) // Zinc 800

val DarkPrimary = Color(0xFF818CF8) // Indigo 400
val DarkOnPrimary = Color(0xFFFFFFFF)

val DarkSecondary = Color(0xFF2DD4BF) // Teal 400
val DarkOnSecondary = Color(0xFF00382E) // Keep dark text on bright secondary

val DarkTextPrimary = Color(0xFFF4F4F5)
val DarkTextSecondary = Color(0xFFA1A1AA)
val DarkOutline = Color(0xFF3F3F46)